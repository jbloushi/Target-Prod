...
async function run() {
        ...
        content: {
            ...
            exportDeclaration: {
                lineItems: [{
                    ...
                    commodityCode: '33030000', // Just as a string as user suggested
                    weight: { netValue: 5, grossValue: 5 },
                    manufacturerCountry: 'KW',
                    exportReasonType: 'permanent' // Testing exportReasonType at line level if needed, but usually it's at exportDeclaration level
                }],
                invoice: {
    ...
    try {
        ...
        fs.writeFileSync('dhl_err2.json', JSON.stringify(err.response?.data, null, 2));
        console.log("ERROR written to dhl_err2.json");
    }
}
run();
