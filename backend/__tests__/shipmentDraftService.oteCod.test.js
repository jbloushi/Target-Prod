describe('ShipmentDraftService OTE COD defaults', () => {
    ...
    it('uses the assigned OTE carrier when request body does not include carrierCode', async () => {
        const ShipmentDraftService = require('../src/services/ShipmentDraftService');
        const targetUser = {
            id: 'client-1',
            role: 'client',
        ...
        prisma.user.findUnique.mockResolvedValue(targetUser);
        prisma.shipment.create.mockImplementation(({ data }) => Promise.resolve(data));

        await ShipmentDraftService.createDraft({
            serviceCode: 'STD',
            currency: 'AED',
            sender: {
                contactPerson: 'Sender',
