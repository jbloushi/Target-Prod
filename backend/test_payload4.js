...
async function run() {
        ...
        content: {
            ...
            exportDeclaration: {
                lineItems: [{
                    ...
                    commodityCodes: [{ typeCode: 'export', value: '33030000' }], // Try 'export' enum?
    ...
    try {
        ...
        console.log("export SUCCESS:", res.status);
    } catch (err) {
        fs.writeFileSync('dhl_err4.json', JSON.stringify(err.response?.data, null, 2));
        console.log("export ERROR written to dhl_err4.json");
    }
}
run();
