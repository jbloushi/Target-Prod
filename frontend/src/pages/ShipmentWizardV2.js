...
import {
    Typography, Button, Box, Grid, Divider, Card,
    CircularProgress, Stack, FormControl, InputLabel,
    Select, MenuItem, alpha, Chip, GlobalStyles,
...
const BookingCard = ({ clients, selectedClient, onClientChange, availableCarriers, selectedCarrier, onCarrierChange }) => (
    <Box sx={{ ...CARD_SX, p: 3, mb: 3 }}>
        ...
        <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={6}>
                <Typography sx={LABEL_SX}>Preferred Carrier</Typography>
                <FormControl fullWidth size="small">
                    <Select
...
const SummaryPanel = ({
    ...
    const fromName = [sender?.company || sender?.contactPerson, sender?.countryCode].filter(Boolean).join(' · ') || '—';
    const toName   = [receiver?.company || receiver?.contactPerson, receiver?.countryCode].filter(Boolean).join(' · ') || 'Pending consignee';
            ...
            <Box sx={{ ...CARD_SX, p: 3, mb: 2 }}>
                ...
                <Stack spacing={1.5} sx={{ mb: 2.5 }}>
                    {[
                        ...
                        { label: 'Freight Type',       value: shipmentType === 'documents' ? 'Document (Non-Dutiable)' : 'Dutiable Package' },
...
const ShipmentWizardV2 = () => {
    ...
    const fetchRates = async () => {
        ...
        try {
            const payload = {
                carrierCode: selectedCarrier, sender, receiver, parcels, items,
                shipmentType, plannedDate,
    ...
    const handleSubmit = async () => {
        ...
        try {
            const payload = {
                ...
                currency, shipmentType, incoterm, pickupRequired,
